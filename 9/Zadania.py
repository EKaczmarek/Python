...
#1
def zad1():
	bufor = list(filter(lambda a: a['height'] != 'unknown', sw_data))
	sorted_data = sorted(bufor, key = lambda a: int(a['height']), reverse = False)
	for i in sorted_data:
		print(i)

#2
def zad2():
	noGender = list(filter(lambda a: a['gender'] == 'n/a', sw_data))
	for i in noGender:
		print(i)
...
#3
def zad3():
	newList = list(filter(lambda a: a['height'] != 'unknown' and a['mass'] != 'unknown' , sw_data))
	maxBmi = max(newList, key = lambda a: ((float(a['mass'].replace(',','.')) / ((float(a['height'].replace(',','.')))/100) **2)))
	print(maxBmi)
	minBmi = min(newList, key = lambda a: ((float(a['mass'].replace(',','.')) / ((float(a['height'].replace(',','.')))/100) **2)))
	print(minBmi)
